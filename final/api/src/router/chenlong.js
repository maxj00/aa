...
module.exports={
    register(app){
        ...
        app.post('/delcarlist',(req,res)=>{
            ...
            var userGetSql = `SELECT * FROM carlist where username='${req.body.username}'`;
        ...
        app.post('/nimabi',(req,res)=>{
            var delSql = `DELETE FROM carlist where username='${req.body.username}'`;
            var userGetSql = `SELECT * FROM carlist where username='${req.body.username}'`;
            db.mysql.select(delSql,function(res2){
                if(res2.status){
                db.mysql.select(userGetSql,function(result){
